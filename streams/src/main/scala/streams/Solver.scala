...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): Stream[(Block, List[Move])] = {
    def loop(neighbors: List[(Block, Move)]): Stream[(Block, List[Move])] = neighbors match {
      case Nil => Stream.empty
      case (block, move) :: tail => (block, move :: history) #:: loop(tail)
    }
  ...
  def actualNeighbors(): Stream[(Block, List[Move])] = {
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    if (initial.isEmpty) Stream.empty
      ...
      val nextInitial: Stream[(Block, List[Move])] = initial.flatMap { case (b, h) =>
         newNeighborsOnly(neighborsWithHistory(b, h), explored)
      }
      initial #::: from(nextInitial, nextInitial.foldLeft(explored) { case (acc, (b, _)) => acc + b })
    }
  }
